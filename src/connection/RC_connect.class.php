...
trait RC_connect {
    public function connect() {
        ...
        $conn = $this->initConnection($host, $port, $errno, $errstr);
        if (!$conn)
...
trait RC_SSL_context {
    ...
    private function getSSLContext () {
        ...
        debug(10, "Create SSL context ", $ssl_context);
        return $ssl_ctx;
    }
}
...
trait RC_connect_Irods {
    ...
    private function initConnection($host, $port, $errno, $errstr) {
...
trait RC_connect_IrodsSSL {
    use RC_connect;

    private function postConnection(resource $conn, RODSMessage $msg) {
